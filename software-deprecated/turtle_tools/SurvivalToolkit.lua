...
function utils.assessTreeFarm(R)
	local _, storage = utils.isStorage("down")
	if storage == "modem" then	-- assume facing farm
		R.networkFarm = true
		T:go("R1F1")
		local blockType = T:getBlockType("down")	-- should be log of farm type
		if blockType:find("log") ~= nil then
			...
			T:go("B1L1")
			R.message = utils.restartTreeFarm()
		end
	else
		R.message = utils.restartTreeFarm()
	end
...
function utils.checkFarmPosition(R, allowEmpty)
	if allowEmpty == nil then allowEmpty = false end
	local discovered = ""
	local success, storage, detected = false, "", 0
	local blockType = T:getBlockType("down")
	if blockType == "" and allowEmpty then
		R.ready = true
		return R
	end
	T:saveToLog("Checking position "..blockType.. " below")
	if blockType:find("water") ~= nil then -- over water, ? facing E (crops)
		...
		if discovered == "barrelbarrel__" or
		   ...
		   discovered == "chestchest__" or
		   discovered == "_modem__" then
		   T:turnLeft(1)
		elseif discovered == "barrel__barrel" or discovered == "chest__chest" or discovered == "modem___" then
		   T:turnLeft(2)
		elseif discovered == "__barrelbarrel" or discovered == "__chestchest" or discovered == "___modem" then
		   T:turnRight(1)
		end
		R.ready = true
	else	-- not over water
...
function utils.checkInventory(inventory, itemName, itemsPerSlot, matchPart)
	--[[
...
function utils.findItemInInventory(inventory, itemName, exactMatch)
	...
	if type(inventory) == "string" then
		--T:saveToLog("utils.findItemInInventory("..inventory..", "..itemName..", exactMatch = "..tostring(exactMatch)..")")
		contents = peripheral.call(inventory, "list")
	else
		T:saveToLog("utils.findItemInInventory(<inventory>, "..itemName..", exactMatch = "..tostring(exactMatch)..")")
		contents = inventory.list()
...
function utils.getItemFromNetwork(storageType, itemRequired, count)
	-- storageType either "chests" or "barrels"
	count = count or 1
	--T:saveToLog("utils.getItemFromNetwork(storageType = "..storageType..", itemRequired = ".. itemRequired..", count = ".. count)
	...
	count = count - turtleCount
	local storage, turtleName, storageNames = utils.wrapModem(R, storageType)	-- list of chest/barrel peripherals, name of turtle, list of storage names
	if count > 0 then -- not enough in stock
		for k, storageName in pairs(storageNames) do 
			local storageSlot, storageCount = 0, 0
			repeat
				storageSlot, storageCount = utils.findItemInInventory(storageName, itemRequired, false)	-- 0,0 if not found
				if storageSlot > 0 then
					if storageCount >= count then
						count = count - utils.sendItemsToTurtle(turtleName, storageName, storageSlot, count)
					else
						count = count - utils.sendItemsToTurtle(turtleName, storageName, storageSlot, storageCount)
					end
				end
				if count <= 0 then
					break
	...
	turtleSlot, turtleCount = T:getItemSlot(itemRequired)
...
function utils.getSlotContains(inventoryName, inSlot)
...
function utils.moveItemsFromTurtle(turtleName, toInventoryName, fromTurtleSlot, quantity, toSlot)
	--[[
	Move quantity of an item from one inventory to another. Turtles MUST use attachedInventory.pullItems()
	eg utils.moveItemsFromTurtle(turtleName, chestName, turtleSlot, turtleCount, nil)
...
function utils.restartTreeFarm()
	return "restartTreeFarm() not yet implemented"
end
...
function utils.sendItemToNetworkChests(R, itemToSend, count)
	local chests, turtleName, chestNames = utils.wrapModem(R, "chest")	-- list of chest peripherals, name of turtle
	local success = false
	local lib = {}
	
	function lib.send(turtleName, chestNames, itemToSend, sourceSlot, count, itemsPerSlot)
		local parts = T:getNames(itemToSend)
		-- "minecraft:crafting_table" -> {"minecraft", "crafting", "table"}
		-- "cobblestone_slab" -> {"cobblestone", "slab"}
		-- "seeds" -> {"seeds"}
		T:saveToLog("key words = "..table.concat(parts, ", "))
		for k, chestName in pairs(chestNames) do				-- iterate storage of selected type
			-- find exact item only
			local inStock, capacity, partMatch = utils.checkInventory(chestName, itemToSend, itemsPerSlot, "")
			T:saveToLog("Trying: "..chestName..", inStock = "..inStock..", capacity = "..capacity)
			if inStock > 0 and capacity >= count then				-- only send if matches existing material
				utils.moveItemsFromTurtle(turtleName, chestName, sourceSlot, count)
				return true
			end
		end
		-- if already transferred, will not reach this code
		for k, chestName in pairs(chestNames) do				-- iterate storage of selected type
			for part = #parts, 1 , -1 do
				-- match only eg, "slab" then "cobblestone" then "minecraft"
				local inStock, capacity, partMatch = utils.checkInventory(chestName, itemToSend, itemsPerSlot, parts[part])
				T:saveToLog("Re-Trying: "..chestName..", inStock = "..inStock..", capacity = "..capacity)
	...
	count = count or 1
	-- turtle is next to modem. MUST remove crafting table if modem on that side. Other tools ok
	-- get turtle name and list of attached chests
	-- chest.pushItems(turtleName, 1, 1) -- pushItems FROM chest, slot, limit TO turtle (, slot)
	-- chest.pullItems(turtleName, 1, 1) -- pullItems FROM turtle, slot ,limit, toSlot
	local sourceSlot = 0
	local itemsPerSlot = 64
	if itemToSend ~= "all" then
		sourceSlot = T:getItemSlot(itemToSend)			-- "all" will give 0
		if sourceSlot == 0 then							-- item not present
			return										-- exit
		end
	end
	--T:saveToLog("storage list =\n"..table.concat(chestNames, ", "))
	T:saveToLog("utils.sendItemToNetworkChests(R, "..itemToSend..", sourceSlot = ".. sourceSlot..", count = "..count..")")

	if itemToSend == "all" then
		T:saveToLog("Emptying turtle inventory")
		for sourceSlot = 1, 16 do
			if turtle.getItemCount(sourceSlot) > 0 then
				itemToSend = T:getItemName(sourceSlot)
				count = turtle.getItemCount(sourceSlot)
				itemsPerSlot = turtle.getItemSpace(sourceSlot) + count
				if itemToSend:find("sapling") ~= nil or itemToSend:find("crafting") ~= nil then
					-- sapling / crafting table should go to barrels
					utils.sendItemToNetworkBarrels(R, itemToSend, count)
				else
					T:saveToLog("lib.send("..turtleName..", <chestNames>, "..itemToSend..", slot = "..sourceSlot..", count = "..count..", itemsPerSlot = "..itemsPerSlot)
					lib.send(turtleName, chestNames, itemToSend, sourceSlot, count, itemsPerSlot)
				end
			end
		end
		if T:isEmpty() then
		...
		T:saveToLog("Sending specific item: "..itemToSend)
		itemsPerSlot = turtle.getItemSpace(sourceSlot) + count
		success = lib.send(turtleName, chestNames, itemToSend, sourceSlot, count, itemsPerSlot)
		T:saveToLog("lib.send("..turtleName..", <chestNames>, "..
					itemToSend..", slot = "..sourceSlot..", count = "..
					count..", itemsPerSlot = "..itemsPerSlot.."): success  = "..tostring(success))
	end
	if not success then
		print("Storage may be full. Remove items")
		print("Press Enter when completed")
		read()
	end
end

function utils.sendItemToNetworkBarrels(R, itemToSend, count)
	-- Must be next to a modem: MUST remove crafting table if modem on that side. Other tools ok
	count = count or 1
	local sourceSlot = T:getItemSlot(itemToSend)
	T:saveToLog("utils.sendItemToNetworkBarrels(R, "..itemToSend.."sourceSlot = "..sourceSlot..", count = "..count..")")
	if sourceSlot > 0 then
		local barrels, turtleName, barrelNames = utils.wrapModem(R, "barrel")	-- list of barrel peripherals, name of turtle
		local destName = barrelNames[1]
		local destBarrel = peripheral.wrap(destName)
		destBarrel.pullItems(turtleName, sourceSlot, count)
	end
end

function utils.sendItemsToTurtle(turtleName, fromInventoryName, fromInventorySlot, quantity, toTurtleSlot)
	--[[
	Move quantity of an item from one inventory to another. Turtles MUST use attachedInventory.pushItems()
	eg utils.sendItemsToTurtle(turtleName, chestName, chestSlot, chestCount, 16) -- move to slot 16 so must be empty
...
function utils.wrapModem(R, storageType)
	...
	--else
		--modem = peripheral.wrap(direction)		-- wrap modem next to turtle (beware crafting table!)
	--end
	local turtleName = modem.getNameLocal()		-- get name of the turtle
	local storage = nil
	if storageType:find("barrel") ~= nil then
		storage = {peripheral.find("minecraft:barrel")}
	else
		storage = {peripheral.find("minecraft:chest")}
	end
	local refined = {}
	local names = {}
	for k, value in ipairs(storage) do
		...
		if not utils.tableContains(names, name, true) then	-- use exact match as checking peripherals
			table.insert(refined, value)
			table.insert(names, name)
		end
	end
	return refined, turtleName, names
end
...
local function createFarmExtension(R) -- 32
	...
	R = utils.checkFarmPosition(R, false)
...
local function createFloorCeiling(R) -- 79 size integer 1 to 4
	...
	function lib.placeRow(R, useBlock, direction, waterPresent)
		for y = 1, R.length do
			local blockType = T:getBlockType("forward")
			...
			T:place(useBlock, direction, false) -- leaveExisting = false
			if y < R.length then
				T:forward(1)
			end
		end
	...
	if R.width == 1 then 					-- single block ahead only
		waterPresent = lib.placeRow(R, useBlock, direction, waterPresent)
		T:turnRight(2)						-- turn at the top of the run
		T:forward(R.length - 1)				-- return to start
		T:turnRight(2)						-- turn round to original position
	else
...
local function harvestTreeFarm(R) -- 24
	...
	function lib.storeInventory(R)
		utils.useSticksAsFuel()	--also uses mangrove roots
		utils.sendItemToNetworkBarrels(R, "propagule", 64)
		utils.sendItemToNetworkBarrels(R, "sapling", 64)
		utils.sendItemToNetworkBarrels(R, "apple", 64)
		utils.sendItemToNetworkChests(R, "all", 0)
	end
	...
	if not T:isEmpty() and R.networkFarm then
		lib.storeInventory(R)
	end
	
	if turtle.getFuelLevel() < turtle.getFuelLimit() / 2 then
		local turtleSlot, turtleCount = utils.getItemFromNetwork("chests", "log", 16)
	...
	if R.networkFarm then			-- monitor trees for growth
		if R.logType == "mangrove" then
			...
			if not turtle.detect() then -- using double trees
				T:forward(1)
...
local function manageFarm(R)
	...
	function lib.getHoe()
		...
		if R.networkFarm then	-- get hoe from storage
			hoeSlot = utils.getItemFromNetwork("barrels", "minecraft:diamond_hoe", 1)
			if hoeSlot == 0 then
...
next to the water source.
 ...
 2. Restart"
	...
	function lib.getSaplings(R, direction)
		...
		if R.networkFarm then
			utils.getItemFromNetwork("barrels", "sapling", 1)
	...
	function lib.getSeeds(R, direction, seed)
		...
		if R.networkFarm then
			inventorySlot, seedCount = utils.getItemFromNetwork("chests", seed, 64)
		else
			while T:suck(direction, 1) do end -- remove saplings/ seeds
	...
	function lib.getVeg(R, direction, crop)
		...
		if R.networkFarm then
			inventorySlot, cropCount = utils.getItemFromNetwork("chests", crop, 64)
		else
	...
	function lib.gotoTree(R)
		...
		if R.networkFarm then
			utils.sendItemToNetworkBarrels(R, "sapling", 64) --move saplings to any attached barrel
		else
	...
	function lib.refuelWithLogs(R, logSlot)
		...
		if R.networkFarm then
			slot = utils.getItemFromNetwork("barrels", "minecraft:crafting_table", 1)
		...
		if T:equip("right", "minecraft:crafting_table") then -- swap equipment on right side
			if R.networkFarm then
				utils.sendItemToNetworkBarrels(R, "minecraft:diamond_hoe", 1)
			end
			...
			--T:unequip("right")
			if R.networkFarm then
				slot = utils.getItemFromNetwork("barrels", "minecraft:diamond_hoe", 1)
			end
			...
			if R.networkFarm then
				utils.sendItemToNetworkBarrels(R, "minecraft:crafting_table", 1)
			else
	...
	function lib.storeCrops(R)
		...
		if R.networkFarm then
			utils.sendItemToNetworkChests(R, "seeds", 0)	-- store seeds in chest(s) dedicated to seeds
			utils.sendItemToNetworkChests(R, "mysticalagriculture:inferium_essence", 0)
			utils.sendItemToNetworkChests(R, "all", 0)	-- store wheat, carrots, beetroot, potatoes in dedicated chest(s)
		else
	...
	R = utils.checkFarmPosition(R, false)	-- should be facing crops, placed above water source. R.ready, R.networkFarm is true/false
	...
	while true do -- start infinite loop of watching crops, farming all modules
		...
		if not R.auto then -- not started from startup.lua
			if R.networkFarm then
				...
				utils.getItemFromNetwork("barrels", "minecraft:crafting_table", 1)
				utils.sendItemToNetworkBarrels(R, "minecraft:diamond_hoe", 1)
...
local function plantTreefarm(R) -- 23
	...
	function lib.getSaplingInventory(R)
		...
		elseif T:getItemSlot("propagule") > 0 then
			...
			if R.networkFarm then
				...
				local turtleSlot, turtleCount = utils.getItemFromNetwork("barrels", R.useBlockType, 16)
				...
				turtleSlot, turtleCount = utils.getItemFromNetwork("barrels", "minecraft:dirt", 16)
				if turtleCount == 0 then
					turtleSlot, turtleCount = utils.getItemFromNetwork("chests", "minecraft:dirt", 16)
				end
	...
	function lib.getMangroveSupplies()
		local turtleSlot, turtleCount = utils.getItemFromNetwork("barrels", "minecraft:dirt", 169)
		if turtleCount < 169 then
			turtleSlot, turtleCount = utils.getItemFromNetwork("chests", "minecraft:dirt", 169 - turtleCount)
			if turtleCount < 169 then	-- ask player for saplings
				T:checkInventoryForItem({"dirt"}, {169 - turtleCount})
			end
		end
		turtleSlot, turtleCount = utils.getItemFromNetwork("barrels", "minecraft:mangrove_propagule", 25)
		if turtleCount == 0 then	-- ask player for saplings
	...
	function lib.plantMangrove()
		T:go("L1F6 R1F1 U1")
		createFloorCeiling({width = 13, length = 13, up = false, down = true, height = 0, subChoice = 0, useBlockType = "minecraft:dirt"})
	...
	if not T:isEmpty() then
		if R.networkFarm then
			...
			utils.sendItemToNetworkBarrels(R, "sapling", 64)
			utils.sendItemToNetworkBarrels(R, "apple", 64)
			utils.sendItemToNetworkBarrels(R, "dirt", 64)
			utils.sendItemToNetworkChests(R, "all", 0)
		end
	end
	if R.subChoice == 3 then	-- mangrove, R.networkFarm only 
		...
		elseif R.subChoice == 2 then -- 4 double trees (4 saplings)
			return {lib.plantDouble(R)}	-- message returned 
		end
	end
	return {}
end
